...
bool is_point_accepted(double r2, double x, double y) {
    return (x * x + y * y) < r2;
}

double pi_monte_carlo(double r2, unsigned int total_points, unsigned int n_threads) {
...
#pragma omp for reduction(+:accepted_points)
        for (i = 0; i < total_points; ++i) {
            ...
            x = get_rand(&seed_x);
            y = get_rand(&seed_y);
            accepted_points += is_point_accepted(r2, x, y);
        }
    }
...
int main() {
    ...
    double pi = pi_monte_carlo(r2, total_points, n_threads);
